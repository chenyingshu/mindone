...
class MaxLongEdgeMinShortEdgeResize(nn.Cell):
    ...
    Args:
        ...
        interpolation (Interpolation Method): Desired interpolation enum defined by
            :class:`mindspore.dataset.vision.Inter`. Default is ``~.Inter.BICUBIC``.
            If input is PIL, only ``~.Inter.NEAREST``, ``~.Inter.ANTIALIAS``,
            ``~.Inter.BILINEAR``, ``~.Inter.CUBIC``, and ``~.Inter.BICUBIC`` are supported.
    ...
    def construct(self, img, img_num=1):
        """
        Args:
            img (PIL Image): Image to be resized.
            img_num (int): Number of images, used to change max_tokens.
        Returns:
            PIL Image: Rescaled image with divisible dimensions.
        """
        ...
        # input is an image of PIL type or a Numpy array in [0, 255] uint8 in the format of <H, W> / <H, W, C>
        return vision.Resize((new_height, new_width), self.interpolation)(img)
...
def motion_blur_opencv(image, kernel_size=15, angle=0):
    ...
    if img.ndim == 2:
        ...
        # 对于彩色图像，各通道独立卷积 Convolute in individual channels
