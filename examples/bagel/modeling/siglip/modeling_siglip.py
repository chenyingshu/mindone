...
def _trunc_normal_(tensor, mean, std, a, b):
    ...
    l_val = norm_cdf((a - mean) / std)
    u = norm_cdf((b - mean) / std)

    # Uniformly fill tensor with values from [l, u], then translate to
    # [2l-1, 2u-1].
    tensor.uniform_(2 * l_val - 1, 2 * u - 1)
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, SiglipVisionEmbeddings):
            ...
            module.q_proj.weight.set_data(
                initializer(XavierUniform(), shape=module.q_proj.weight.shape, dtype=module.q_proj.weight.dtype)
            )
            module.k_proj.weight.set_data(
                initializer(XavierUniform(), shape=module.k_proj.weight.shape, dtype=module.k_proj.weight.dtype)
            )
            module.v_proj.weight.set_data(
                initializer(XavierUniform(), shape=module.v_proj.weight.shape, dtype=module.v_proj.weight.dtype)
            )
            module.out_proj.weight.set_data(
                initializer(XavierUniform(), shape=module.out_proj.weight.shape, dtype=module.out_proj.weight.dtype)
            )
            module.q_proj.bias.set_data(
                initializer("zeros", shape=module.q_proj.bias.shape, dtype=module.q_proj.bias.dtype)
            )
            module.k_proj.bias.set_data(
                initializer("zeros", shape=module.k_proj.bias.shape, dtype=module.k_proj.bias.dtype)
            )
            module.v_proj.bias.set_data(
                initializer("zeros", shape=module.v_proj.bias.shape, dtype=module.v_proj.bias.dtype)
            )
            module.out_proj.bias.set_data(
                initializer("zeros", shape=module.out_proj.bias.shape, dtype=module.out_proj.bias.dtype)
            )
        elif isinstance(module, SiglipMLP):
            module.fc1.weight.set_data(
                initializer(XavierUniform(), shape=module.fc1.weight.shape, dtype=module.fc1.weight.dtype)
            )
            module.fc2.weight.set_data(
                initializer(XavierUniform(), shape=module.fc2.weight.shape, dtype=module.fc2.weight.dtype)
            )
            module.fc1.bias.set_data(
                initializer(
            ...
            module.attention.in_proj_bias.set_data(
                initializer(
                    "zeros", shape=module.attention.in_proj_bias.shape, dtype=module.attention.in_proj_bias.dtype
                )
            )
        elif isinstance(module, SiglipModel):
            logit_scale_init = mint.log(ms.tensor(1.0))
            scale_weight = initializer(
                Constant(logit_scale_init), shape=module.logit_scale.shape, dtype=module.logit_scale.dtype
            )
            module.logit_scale.set_data(scale_weight)
            module.logit_bias.set_data(
                initializer("zeros", shape=module.logit_bias.shape, dtype=module.logit_bias.dtype)
            )
        elif isinstance(module, SiglipForImageClassification):
            module.classifier.weight.set_data(
                initializer(
...
class SiglipModel(SiglipPreTrainedModel):
    ...
    def construct(
        ...
        logits_per_text = (
            mint.matmul(text_embeds, image_embeds.swapaxes(0, 1)) * self.logit_scale.exp() + self.logit_bias
        )
        ...
        if return_loss:
            # Adapted from
            # https://github.com/google-research/big_vision/blob/01edb81a4716f93a48be43b3a4af14e29cdb3a7f/big_vision/trainers/proj/image_text/siglip.py#L287
