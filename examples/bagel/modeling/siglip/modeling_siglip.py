...
from mindspore.common.initializer import Constant, HeNormal, Uniform, initializer, Normal
...
from mindone.transformers.modeling_outputs import BaseModelOutput, BaseModelOutputWithPooling, ImageClassifierOutput
...
if is_flash_attn_2_available():
    from mindone.transformers.integrations.flash_attention import flash_attention_forward as _flash_attention_forward
...
def _trunc_normal_(tensor, mean, std, a, b):
    # Cut & paste from MindSpore official master until it's in a few official releases - RW
...
def variance_scaling_(tensor, scale=1.0, mode="fan_in", distribution="normal"):
    fan_in, fan_out = initializer._calculate_fan_in_and_fan_out(tensor.shape)
    if mode == "fan_in":
        denom = fan_in
    elif mode == "fan_out":
        denom = fan_out
    ...
    if distribution == "truncated_normal":
        ...
        tensor = trunc_normal_tf_(tensor, std=math.sqrt(variance) / 0.87962566103423978)
...
class SiglipFlashAttention2(SiglipAttention):
    ...
    def construct(
        ...
        attention_mask = convert_mask_to_fa_format(attention_mask)
...
SIGLIP_ATTENTION_CLASSES = {
    ...
    "sdpa": SiglipAttention, # not supported
}
...
class SiglipPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, SiglipVisionEmbeddings):
            ...
            weight = initializer(
                Normal(sigma=1.0 / np.sqrt(width), mean=0.0),
                shape=module.position_embedding.weight.shape,
                dtype=module.position_embedding.dtype,
            )
            module.position_embedding.weight.set_data(weight)
        elif isinstance(module, mint.nn.Embedding):
            default_flax_embed_init(module.weight)
        elif isinstance(module, SiglipAttention):
            module.q_proj.weight.set_data(initializer(XavierUniform(), shape=module.q_proj.weight.shape, dtype=module.q_proj.weight.dtype))
            module.k_proj.weight.set_data(initializer(XavierUniform(), shape=module.k_proj.weight.shape, dtype=module.k_proj.weight.dtype))
            module.v_proj.weight.set_data(initializer(XavierUniform(), shape=module.v_proj.weight.shape, dtype=module.v_proj.weight.dtype))
            module.out_proj.weight.set_data(initializer(XavierUniform(), shape=module.out_proj.weight.shape, dtype=module.out_proj.weight.dtype))
            module.q_proj.bias.set_data(initializer("zeros", shape=module.q_proj.bias.shape, dtype=module.q_proj.bias.dtype))
            module.k_proj.bias.set_data(initializer("zeros", shape=module.k_proj.bias.shape, dtype=module.k_proj.bias.dtype))
            module.v_proj.bias.set_data(initializer("zeros", shape=module.v_proj.bias.shape, dtype=module.v_proj.bias.dtype))
            module.out_proj.bias.set_data(initializer("zeros", shape=module.out_proj.bias.shape, dtype=module.out_proj.bias.dtype))
        elif isinstance(module, SiglipMLP):
            module.fc1.weight.set_data(initializer(XavierUniform(), shape=module.fc1.weight.shape, dtype=module.fc1.weight.dtype))
            module.fc2.weight.set_data(initializer(XavierUniform(), shape=module.fc2.weight.shape, dtype=module.fc2.weight.dtype))
            module.fc1.bias.set_data(initializer(
                Normal(sigma=1e-6, mean=0.0),
                shape=module.fc1.bias.shape,
                dtype=module.fc1.bias.dtype,
            ))
            module.fc2.bias.set_data(initializer(
                Normal(sigma=1e-6, mean=0.0),
                shape=module.fc2.bias.shape,
                dtype=module.fc2.bias.dtype,
            ))
        elif isinstance(module, SiglipMultiheadAttentionPoolingHead):
            module.probe.set_data(initializer(XavierUniform(), shape=module.probe.shape, dtype=module.probe.dtype))
            module.attention.in_proj_weight.set_data(
                initializer(
                    XavierUniform(),
                    shape=module.attention.in_proj_weight.shape,
                    dtype=module.attention.in_proj_weight.dtype
                )
            )
            module.attention.in_proj_bias.set_data(initializer("zeros", shape=module.attention.in_proj_bias.shape, dtype=module.attention.in_proj_bias.dtype))
        elif isinstance(module, SiglipModel):
            logit_scale_init = mint.log(ms.tensor(1.0))
            scale_weight = initializer(Constant(logit_scale_init), shape=module.logit_scale.shape, dtype=module.logit_scale.dtype)
            module.logit_scale.set_data(scale_weight)
            module.logit_bias.set_data(initializer("zeros", shape=module.logit_bias.shape, dtype=module.logit_bias.dtype))
        elif isinstance(module, SiglipForImageClassification):
            module.classifier.weight.set_data(initializer(
                Normal(sigma=self.config.vision_config.hidden_size**-0.5 * self.config.initializer_factor),
                shape=module.classifier.weight.shape,
            ...
            module.bias.set_data(initializer("zeros", shape=module.bias.shape, dtype=module.bias.dtype))
            module.weight.set_data(initializer("ones", shape=module.weight.shape, dtype=module.weight.dtype))
...
class SiglipTextTransformer(nn.Cell):
    def __init__(self, config: SiglipTextConfig):
        ...
        self.head = mint.nn.Linear(embed_dim, embed_dim)
    ...
    def construct(
        ...
        if attention_mask is not None: # and not self._use_flash_attention_2:
            ...
            attention_mask = _prepare_4d_attention_mask(attention_mask, ms.float16)
...
class SiglipMultiheadAttentionPoolingHead(nn.Cell):
    ...
    def __init__(self, config: SiglipVisionConfig):
        ...
        self.probe = nn.Parameter(mint.randn(1, 1, config.hidden_size))
...
class SiglipModel(SiglipPreTrainedModel):
    ...
    def __init__(self, config: SiglipConfig):
        ...
        self.logit_scale = nn.Parameter(mint.randn(1))
        self.logit_bias = nn.Parameter(mint.randn(1))
    ...
    def construct(
        ...
        # normalized features
        image_embeds = image_embeds / image_embeds.norm(ord="fro", dim=-1, keepdim=True)
        text_embeds = text_embeds / text_embeds.norm(ord="fro", dim=-1, keepdim=True)

        # cosine similarity as logits
        logits_per_text = (
            mint.matmul(text_embeds, image_embeds.swapaxes(0,1)) * self.logit_scale.exp()
            + self.logit_bias
        )
        logits_per_image = logits_per_text.swapaxes(0,1)
        ...
        if return_loss:
            ...
            eye = mint.eye(logits_per_text.shape[0])
...
class SiglipForImageClassification(SiglipPreTrainedModel):
    ...
    def __init__(self, config: SiglipConfig) -> None:
        ...
        # Classifier head
        self.classifier = (
            mint.nn.Linear(config.vision_config.hidden_size, config.num_labels) if config.num_labels > 0 else mint.nn.Identity()
        )
