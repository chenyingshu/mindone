...
class Bagel(PreTrainedModel):
    ...
    def construct(
        self,
        sequence_length: int,
        packed_text_ids: ms.Tensor,
        packed_text_indexes: ms.Tensor,
        sample_lens: List[int],
        packed_position_ids: ms.Tensor,
        nested_attention_masks: List[ms.Tensor] = None,
        split_lens: List[int] = None,
        attn_modes: List[str] = None,
        # for visual understanding
        ce_loss_indexes: Optional[ms.Tensor] = None,
        packed_label_ids: Optional[ms.Tensor] = None,
        packed_vit_tokens: Optional[ms.Tensor] = None,
        packed_vit_token_indexes: Optional[ms.Tensor] = None,
        packed_vit_position_ids: Optional[ms.Tensor] = None,
        vit_token_seqlens: Optional[ms.Tensor] = None,
        # for visual generation
        padded_latent: Optional[ms.Tensor] = None,
        patchified_vae_latent_shapes: Optional[List[Tuple[int, int]]] = None,
        packed_latent_position_ids: Optional[ms.Tensor] = None,
        packed_vae_token_indexes: Optional[ms.Tensor] = None,
        packed_timesteps: Optional[ms.Tensor] = None,
        mse_loss_indexes: Optional[ms.Tensor] = None,
    ) -> ms.Tensor:
        """
        Args:
        ...
        # if nested_attention_masks is None: # TODO: not supported
        #     sparse_mask = create_sparse_mask(sample_lens, split_lens, attn_modes)
        ...
        if self.config.visual_und:
            ...
            cu_seqlens = cu_seqlens.to(ms.int32)
    ...
    def prepare_prompts(self, curr_kvlens, curr_rope, prompts, tokenizer, new_token_ids):
        ...
        generation_input = {
            "text_token_lens": ms.tensor(text_token_lens, dtype=ms.int32),
            "packed_text_ids": ms.tensor(packed_text_ids, dtype=ms.int64),
            "packed_text_position_ids": ms.tensor(packed_text_position_ids, dtype=ms.int64),
            "packed_text_indexes": ms.tensor(packed_text_indexes, dtype=ms.int64),
            "packed_key_value_indexes": ms.tensor(packed_key_value_indexes, dtype=ms.int64),
            "key_values_lens": ms.tensor(curr_kvlens, dtype=ms.int32),
        }
    ...
    def forward_cache_update_text(
        ...
        packed_text_ids: ms.Tensor,
        packed_text_position_ids: ms.Tensor,
        text_token_lens: ms.Tensor,
        packed_text_indexes: ms.Tensor,
        packed_key_value_indexes: ms.Tensor,
        key_values_lens: ms.Tensor,
    ):
        with no_grad():
            packed_text_embedding = self.language_model.model.embed_tokens(packed_text_ids)
    ...
    def prepare_vit_images(self, curr_kvlens, curr_rope, images, transforms, new_token_ids):
        ...
        generation_input = {
            "packed_text_ids": ms.tensor(packed_text_ids, dtype=ms.int64),
            "packed_text_indexes": ms.tensor(packed_text_indexes, dtype=ms.int64),
            "vit_token_seqlens": ms.tensor(vit_token_seqlens, dtype=ms.int32),
            "packed_vit_tokens": mint.cat(packed_vit_tokens, dim=0),
            "packed_vit_position_ids": mint.cat(packed_vit_position_ids, dim=0),
            "packed_vit_token_indexes": ms.tensor(packed_vit_token_indexes, dtype=ms.int64),
            "packed_position_ids": ms.tensor(packed_position_ids, dtype=ms.int64),
            "packed_seqlens": ms.tensor(packed_seqlens, dtype=ms.int32),
            "packed_indexes": ms.tensor(packed_indexes, dtype=ms.int64),
            "packed_key_value_indexes": ms.tensor(packed_key_value_indexes, dtype=ms.int64),
            "key_values_lens": ms.tensor(curr_kvlens, dtype=ms.int32),
        }
    ...
    def forward_cache_update_vit(
        ...
        packed_text_ids: ms.Tensor,
        packed_text_indexes: ms.Tensor,
        packed_vit_tokens: ms.Tensor,
        packed_vit_token_indexes: ms.Tensor,
        packed_vit_position_ids: ms.Tensor,
        vit_token_seqlens: ms.Tensor,
        packed_position_ids: ms.Tensor,
        packed_seqlens: ms.Tensor,
        packed_indexes: ms.Tensor,
        packed_key_value_indexes: ms.Tensor,
        key_values_lens: ms.Tensor,
    ):
        with no_grad():
            ...
            packed_vit_token_embed = ops.stop_gradient(self.connector(packed_vit_token_embed))
            pos_emb = ops.stop_gradient(self.vit_pos_embed(packed_vit_position_ids))
    ...
    def prepare_vae_images(self, curr_kvlens, curr_rope, images, transforms, new_token_ids, timestep=0):
        ...
        generation_input = {
            ...
            "packed_vae_position_ids": mint.cat(packed_vae_position_ids, dim=0),
            "packed_timesteps": ms.tensor([timestep]),
            "packed_vae_token_indexes": ms.tensor(packed_vae_token_indexes, dtype=ms.int64),
            "packed_text_ids": ms.tensor(packed_text_ids, dtype=ms.int64),
            "packed_text_indexes": ms.tensor(packed_text_indexes, dtype=ms.int64),
            "packed_position_ids": ms.tensor(packed_position_ids, dtype=ms.int64),
            "packed_seqlens": ms.tensor(packed_seqlens, dtype=ms.int32),
    ...
    def forward_cache_update_vae(
        ...
        vae_model,
        past_key_values: NaiveCache,
        padded_images: ms.Tensor,
        patchified_vae_latent_shapes: List,
        packed_vae_position_ids: ms.Tensor,
        packed_timesteps: ms.Tensor,
        packed_vae_token_indexes: ms.Tensor,
        packed_text_ids: ms.Tensor,
        packed_text_indexes: ms.Tensor,
        packed_position_ids: ms.Tensor,
        packed_seqlens: ms.Tensor,
        ...
        with no_grad():
            ...
            packed_pos_embed = ops.stop_gradient(self.latent_pos_embed(packed_vae_position_ids))
            packed_timestep_embeds = ops.stop_gradient(self.time_embedder(packed_timesteps))
            packed_latent = ops.stop_gradient(self.vae2llm(packed_latent)) + packed_timestep_embeds + packed_pos_embed
            packed_sequence[packed_vae_token_indexes] = packed_latent
    ...
    def prepare_vae_latent(self, curr_kvlens, curr_rope, image_sizes, new_token_ids):
        ...
        generation_input = {
            "packed_text_ids": ms.tensor(packed_text_ids, dtype=ms.int64),
            "packed_text_indexes": ms.tensor(packed_text_indexes, dtype=ms.int64),
            "packed_init_noises": mint.cat(packed_init_noises, dim=0),
            "packed_vae_position_ids": mint.cat(packed_vae_position_ids, dim=0),
            "packed_vae_token_indexes": ms.tensor(packed_vae_token_indexes, dtype=ms.int64),
            "packed_seqlens": ms.tensor(packed_seqlens, dtype=ms.int32),
            "packed_position_ids": ms.tensor(packed_position_ids, dtype=ms.int64),
    ...
    def prepare_vae_latent_cfg(self, curr_kvlens, curr_rope, image_sizes):
        ...
        generation_input = {
            "cfg_packed_position_ids": ms.tensor(packed_position_ids, dtype=ms.int64),
            "cfg_key_values_lens": ms.tensor(curr_kvlens, dtype=ms.int32),
            "cfg_packed_query_indexes": ms.tensor(packed_indexes, dtype=ms.int64),
            "cfg_packed_key_value_indexes": ms.tensor(packed_key_value_indexes, dtype=ms.int64),
        }
    ...
    def generate_image(
        ...
        cfg_text_packed_query_indexes: Optional[ms.Tensor] = None,
        cfg_text_packed_position_ids: Optional[ms.Tensor] = None,
        cfg_text_past_key_values: Optional[NaiveCache] = None,
        cfg_text_key_values_lens: Optional[ms.Tensor] = None,
        cfg_text_packed_key_value_indexes: Optional[ms.Tensor] = None,
        # cfg_img
        cfg_img_scale: float = 1.0,
        cfg_img_packed_query_indexes: Optional[ms.Tensor] = None,
        cfg_img_packed_position_ids: Optional[ms.Tensor] = None,
        cfg_img_past_key_values: Optional[NaiveCache] = None,
        cfg_img_key_values_lens: Optional[ms.Tensor] = None,
        cfg_img_packed_key_value_indexes: Optional[ms.Tensor] = None,
    ...
    def _forward_flow(
        ...
        with no_grad():
            ...
            packed_pos_embed = ops.stop_gradient(self.latent_pos_embed(packed_vae_position_ids))
            packed_timestep_embeds = ops.stop_gradient(self.time_embedder(timestep))
            ...
            if cfg_text_scale > 1.0:
                if cfg_renorm_type == "text_channel":
                    ...
                    norm_v_t = mint.norm(v_t, dim=-1, keepdim=True)
                    norm_v_t_text_ = mint.norm(v_t_text_, dim=-1, keepdim=True)
                    ...
                    if cfg_renorm_type == "global":
                        norm_v_t = mint.norm(v_t)
                        norm_v_t_ = mint.norm(v_t_)
                    elif cfg_renorm_type == "channel":
                        norm_v_t = mint.norm(v_t, dim=-1, keepdim=True)
                        norm_v_t_ = mint.norm(v_t_, dim=-1, keepdim=True)
                    else:
    ...
    def prepare_start_tokens(self, curr_kvlens, curr_rope, new_token_ids):
        ...
        generation_input = {
            "packed_start_tokens": ms.tensor(packed_start_tokens, dtype=ms.int64),
            "packed_query_position_ids": ms.tensor(packed_query_position_ids, dtype=ms.int64),
            "key_values_lens": ms.tensor(curr_kvlens, dtype=ms.int32),
            "packed_key_value_indexes": ms.tensor(packed_key_value_indexes, dtype=ms.int64),
        }
    ...
    def generate_text(
        ...
        packed_key_value_indexes: ms.Tensor,
        key_values_lens: ms.Tensor,
        packed_start_tokens: ms.Tensor,
        packed_query_position_ids: ms.Tensor,
        max_length: int,
        ...
        with no_grad():
            ...
            while step < max_length:
                ...
                if do_sample:
                    probs = ops.stop_gradient(F.softmax(pred_logits / temperature, dim=-1))
                    curr_tokens = ops.stop_gradient(mint.multinomial(probs, num_samples=1).squeeze(1))
                else:
                    curr_tokens = ops.stop_gradient(mint.argmax(pred_logits, dim=-1))
        ...
        with no_grad():
            if isinstance(new_token_ids, dict):
                for k, v in new_token_ids.items():
                    if isinstance(v, ms.Tensor):
                        new_token_ids[k] = v
            elif isinstance(new_token_ids, ms.Tensor):
            ...
            for image in images:
                ...
                for k, v in generation_input.items():
                    if isinstance(v, ms.Tensor):
                        generation_input[k] = v
                # with torch.amp.autocast("cuda", enabled=True, dtype=ms.bfloat16):
