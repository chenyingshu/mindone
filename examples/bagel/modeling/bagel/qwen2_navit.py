...
class BaseNavitOutputWithPast(ModelOutput):
    packed_query_sequence: ms.Tensor = None
...
def pad_sequence(tensor, pad_size):
    ...
    return mint.cat([tensor, pad_tensor], dim=1)
...
class PackedAttention(Qwen2Attention):
    def __init__(self, config, layer_idx: Optional[int] = None):
        ...
        if self.config.qk_norm:
            ...
            self.q_norm = mint.nn.Identity()
            self.k_norm = mint.nn.Identity()
    ...
    def forward_train(
        self,
        packed_sequence: ms.Tensor,
        sample_lens: List[int],
        attention_mask: List[ms.Tensor],
        packed_position_embeddings: Tuple[ms.Tensor, ms.Tensor],
    ):
        ...
        if isinstance(attention_mask, List):
            packed_key_states = packed_key_states[:, :, None, :].tile((1, 1, self.num_key_value_groups, 1))
            packed_key_states = packed_key_states.reshape(-1, self.num_heads, self.head_dim)
            packed_value_states = packed_value_states[:, :, None, :].tile((1, 1, self.num_key_value_groups, 1))
            packed_value_states = packed_value_states.reshape(-1, self.num_heads, self.head_dim)

            unpacked_query_states = packed_query_states.swapaxes(0, 1).split(sample_lens, dim=1)
            unpacked_key_states = packed_key_states.swapaxes(0, 1).split(sample_lens, dim=1)
            unpacked_value_states = packed_value_states.swapaxes(0, 1).split(sample_lens, dim=1)
            upacked_attn_output = []
            for query_states, key_states, value_states, attention_mask_per_sample in zip(
                ...
                #     attn_mask=attention_mask.to(ms.bfloat16).unsqueeze(0),
                # )
                # For `attn_mask` of ops.flash_attention_score, False indicates retention and True indicates discard,
                # Which is opposite to PyTorch
                attention_mask = (
                    mint.logical_not(attention_mask_per_sample)
        ...
        # else: # flex attention is not supported
        ...
        packed_attn_output = packed_attn_output.swapaxes(0, 1).reshape(-1, self.hidden_size)
    ...
    def forward_inference(
        self,
        packed_query_sequence: ms.Tensor,
        query_lens: ms.Tensor,
        packed_query_position_embeddings: ms.Tensor,
        packed_query_indexes: ms.Tensor,
        past_key_values: Optional[NaiveCache] = None,
        key_values_lens: Optional[ms.Tensor] = None,
        packed_key_value_indexes: Optional[ms.Tensor] = None,
        ...
        packed_key_states = packed_key_states.to(ms.bfloat16)
        packed_value_states = packed_value_states.to(ms.bfloat16)

        if past_key_values is not None and past_key_values.key_cache[self.layer_idx] is not None:
            past_key_states = past_key_values.key_cache[self.layer_idx]
            past_value_states = past_key_values.value_cache[self.layer_idx]

            seqlens = (sum(query_lens) + sum(key_values_lens)).item()
...
class PackedAttentionMoT(Qwen2Attention):
    def __init__(self, config, layer_idx: Optional[int] = None):
        ...
        if self.config.qk_norm:
            ...
            self.q_norm = mint.nn.Identity()
            self.k_norm = mint.nn.Identity()
            self.q_norm_moe_gen = mint.nn.Identity()
            self.k_norm_moe_gen = mint.nn.Identity()

        self.q_proj_moe_gen = mint.nn.Linear(self.hidden_size, self.num_heads * self.head_dim, bias=True)
    ...
    def forward_train(
        ...
        attention_mask,
        packed_position_embeddings: Tuple[ms.Tensor, ms.Tensor],
        packed_und_token_indexes: ms.Tensor,
        packed_gen_token_indexes: ms.Tensor,
    ):
        ...
        if self.config.freeze_und:
            ...
            # FIXME: not sure here
            packed_value_states[packed_und_token_indexes].requires_grad = False
        ...
        if self.config.freeze_und:
            ...
            packed_query_states_[packed_und_token_indexes].requires_grad = False
        ...
        if self.config.freeze_und:
            ...
            packed_key_states_[packed_und_token_indexes].requires_grad = False
        ...
        if isinstance(attention_mask, List):
            packed_key_states_ = packed_key_states_[:, :, None, :].tile((1, 1, self.num_key_value_groups, 1))
            packed_key_states_ = packed_key_states_.reshape(-1, self.num_heads, self.head_dim)
            packed_value_states = packed_value_states[:, :, None, :].tile((1, 1, self.num_key_value_groups, 1))
            packed_value_states = packed_value_states.reshape(-1, self.num_heads, self.head_dim)

            unpacked_query_states = packed_query_states_.swapaxes(0, 1).split(sample_lens, dim=1)
            unpacked_key_states = packed_key_states_.swapaxes(0, 1).split(sample_lens, dim=1)
            unpacked_value_states = packed_value_states.swapaxes(0, 1).split(sample_lens, dim=1)
            upacked_attn_output = []
            for query_states, key_states, value_states, attention_mask_per_sample in zip(
                unpacked_query_states, unpacked_key_states, unpacked_value_states, attention_mask
        ...
        packed_attn_output = packed_attn_output.swapaxes(0, 1).reshape(-1, self.num_heads * self.head_dim)
    ...
    def forward_inference(
        ...
        if mode == "und":
            ...
            packed_query_sequence = packed_query_sequence.to(ms.bfloat16)
            ...
            packed_query_states = packed_query_states.to(ms.float32)
            ...
            packed_key_states = packed_key_states.to(ms.float32)
...
class Qwen2MoTDecoderLayer(nn.Cell):
    def __init__(
        self,
        config,
    ...
    def forward_train(
        ...
        if self.freeze_und:
            # packed_sequence_[packed_und_token_indexes] = packed_sequence_[packed_und_token_indexes].detach()
            packed_sequence_[packed_und_token_indexes].requires_grad = False
    ...
    def forward_inference(
        ...
        if mode == "und":
            ...
            packed_query_sequence_ = mint.zeros_like(packed_query_sequence)
        ...
        if mode == "und":
            ...
            packed_text_query_sequence = packed_query_sequence[packed_text_indexes]
            packed_vae_query_sequence = packed_query_sequence[packed_vae_token_indexes]
            packed_text_query_sequence = self.post_attention_layernorm(packed_text_query_sequence).to(ms.bfloat16)
            packed_vae_query_sequence = self.post_attention_layernorm_moe_gen(packed_vae_query_sequence).to(ms.bfloat16)

            packed_query_sequence_ = mint.zeros_like(packed_query_sequence).to(ms.bfloat16)
...
class Qwen2Model(Qwen2PreTrainedModel):
    def __init__(self, config):
        ...
        self.layers = nn.CellList([layer_module(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
    ...
    def forward_train(
        ...
        packed_und_token_indexes: Optional[ms.Tensor] = None,
        packed_gen_token_indexes: Optional[ms.Tensor] = None,
    ) -> ms.Tensor:
        if self.config.freeze_und:
        ...
        if self.use_moe:
            packed_sequence_ = mint.zeros_like(packed_sequence)
    ...
    def init_moe(self):
        for name, param in self.parameters_and_names():
            if "moe_gen" in name:
                ...
                param.data.copy_(self.state_dict()[original_name].data)  # TODO: not sure
                # param.set_data(self.state_dict()[original_name]) # TODO
