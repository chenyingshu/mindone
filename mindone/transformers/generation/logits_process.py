...
from ..modeling_attn_mask_utils import dtype_to_min, dtype_to_max
...
class MinLengthLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        eos_token_mask = ms.numpy.isin(vocab_tensor, self.eos_token_id)
...
class MinNewTokensLengthLogitsProcessor(LogitsProcessor):
    ...
    >>> inputs = tokenizer(["A number:"], return_tensors="np")
    >>> gen_out = model.generate(Tensor(inputs.input_ids))
    >>> print(tokenizer.batch_decode(gen_out, skip_special_tokens=True)[0])
    A number: one

    >>> # setting `min_new_tokens` will force the model to generate beyond its natural ending point, which is not
    >>> # necessarily incorrect
    >>> gen_out = model.generate(Tensor(inputs.input_ids), min_new_tokens=2)
    >>> print(tokenizer.batch_decode(gen_out, skip_special_tokens=True)[0])
...
class TemperatureLogitsWarper(LogitsProcessor):
    ...
    >>> outputs = model.generate(Tensor(inputs.input_ids), **generate_kwargs)
...
class RepetitionPenaltyLogitsProcessor(LogitsProcessor):
    ...
    >>> summary_ids = model.generate(Tensor(inputs.input_ids))
    >>> print(tokenizer.batch_decode(summary_ids, skip_special_tokens=True)[0])
    I'm not going to be able to do that. I'm going to be able to do that

    >>> # This generates a penalty for repeated tokens
    >>> penalized_ids = model.generate(Tensor(inputs.input_ids), repetition_penalty=1.1)
...
class EncoderRepetitionPenaltyLogitsProcessor(LogitsProcessor):
    ...
    >>> gen_out = model.generate(Tensor(inputs.input_ids), encoder_repetition_penalty=1.2)
    >>> print(tokenizer.batch_decode(gen_out, skip_special_tokens=True)[0])
...
class TopPLogitsWarper(LogitsProcessor):
    ...
    >>> outputs = model.generate(Tensor(inputs.input_ids), do_sample=True)
    ...
    >>> outputs = model.generate(Tensor(inputs.input_ids), do_sample=True, top_p=0.1)
    >>> print(tokenizer.batch_decode(outputs, skip_special_tokens=True)[0])
...
class TopKLogitsWarper(LogitsProcessor):
    ...
    >>> outputs = model.generate(Tensor(inputs.input_ids), do_sample=True, top_k=2)
    >>> print(tokenizer.batch_decode(outputs, skip_special_tokens=True)[0])
...
class MinPLogitsWarper(LogitsProcessor):
    ...
    >>> outputs = model.generate(Tensor(inputs.input_ids), do_sample=True, min_p=0.1)
    >>> print(tokenizer.batch_decode(outputs, skip_special_tokens=True)[0])
    A sequence: 1, 2, 3, 4, 5, 6, 7, 8, 9
    ```
    """
...
class TypicalLogitsWarper(LogitsProcessor):
    ...
    >>> outputs = model.generate(Tensor(inputs.input_ids))
    >>> print(tokenizer.batch_decode(outputs, skip_special_tokens=True)[0])
    ...
    ...     Tensor(inputs.input_ids), do_sample=True, typical_p=0.1, return_dict_in_generate=True, output_scores=True
    ... )
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        sorted_logits = scores.gather_elements(-1, sorted_indices)
        cumulative_probs = sorted_logits.softmax(axis=-1).cumsum(axis=-1)
...
class EpsilonLogitsWarper(LogitsProcessor):
    ...
    >>> outputs = model.generate(Tensor(inputs.input_ids), do_sample=True, epsilon_cutoff=0.1)
    >>> print(tokenizer.batch_decode(outputs, skip_special_tokens=True)[0])
    A sequence: 1, 2, 3, 4, 5, 6, 7, 8, 9
    ```
    """
...
class EtaLogitsWarper(LogitsProcessor):
    ...
    >>> outputs = model.generate(Tensor(inputs.input_ids), do_sample=True, eta_cutoff=0.1)
    >>> print(tokenizer.batch_decode(outputs, skip_special_tokens=True)[0])
    A sequence: 1, 2, 3, 4, 5, 6, 7, 8, 9
    ```
    """
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        # mindspore Categorical does not support logits input yet
        entropy = nn.probability.distribution.Categorical(probs = probabilities).entropy()
...
class EncoderNoRepeatNGramLogitsProcessor(LogitsProcessor):
    ...
    >>> outputs = model.generate(Tensor(inputs.input_ids), encoder_no_repeat_ngram_size=2)
    >>> print(tokenizer.batch_decode(outputs, skip_special_tokens=True)[0])
    Alice: I love cats. What do you love?
...
class SequenceBiasLogitsProcessor(LogitsProcessor):
    ...
    >>> summary_ids = model.generate(Tensor(inputs["input_ids"]), max_new_tokens=4)
    >>> print(tokenizer.batch_decode(summary_ids, skip_special_tokens=True)[0])
    ...
    >>> biased_ids = model.generate(Tensor(inputs["input_ids"]）, max_new_tokens=4, sequence_bias=sequence_bias)
    >>> print(tokenizer.batch_decode(biased_ids, skip_special_tokens=True)[0])
    The full name of Donald is Donald J. Donald,

    >>> biased_ids = model.generate(Tensor(inputs["input_ids"]）, max_new_tokens=4, num_beams=4, sequence_bias=sequence_bias)
    >>> print(tokenizer.batch_decode(biased_ids, skip_special_tokens=True)[0])
...
class NoBadWordsLogitsProcessor(SequenceBiasLogitsProcessor):
    ...
    >>> output_ids = model.generate(Tensor(inputs["input_ids"]), max_new_tokens=5, pad_token_id=tokenizer.eos_token_id)
    ...
    ...     Tensor(inputs["input_ids"]), max_new_tokens=5, bad_words_ids=bad_words_ids, pad_token_id=tokenizer.eos_token_id
    ... )
    >>> print(tokenizer.batch_decode(output_ids, skip_special_tokens=True)[0])
...
class PrefixConstrainedLogitsProcessor(LogitsProcessor):
    ...
    >>> outputs = model.generate(Tensor(inputs.input_ids), max_new_tokens=5)
    >>> print(tokenizer.batch_decode(outputs, skip_special_tokens=True)[0])
    ...
    >>> outputs = model.generate(Tensor(inputs.input_ids), max_new_tokens=5, prefix_allowed_tokens_fn=prefix_allowed_tokens_fn)
    >>> print(tokenizer.batch_decode(outputs, skip_special_tokens=True)[0])
...
class HammingDiversityLogitsProcessor(LogitsProcessor):
    ...
    Note that this logits processor is only effective for [`MSPreTrainedModel.group_beam_search`]. See [Diverse Beam
...
class ForcedBOSTokenLogitsProcessor(LogitsProcessor):
    ...
    >>> # By default, it continues generating according to the model's logits
    >>> outputs = model.generate(Tensor(inputs.input_ids), max_new_tokens=10)
    ...
    >>> outputs = model.generate(Tensor(inputs.input_ids), max_new_tokens=10, forced_bos_token_id=tokenizer.eos_token_id)
    >>> print(tokenizer.batch_decode(outputs)[0])
    <pad></s>
    ```
    """
...
class ForcedEOSTokenLogitsProcessor(LogitsProcessor):
    ...
    >>> outputs = model.generate(Tensor(inputs.input_ids), max_new_tokens=10, forced_eos_token_id=tokenizer.eos_token_id)
    >>> print(tokenizer.batch_decode(outputs)[0])
...
class InfNanRemoveLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        scores_processed = ops.where(scores == float("inf"), dtype_to_max(scores.dtype), scores_processed)
        scores_processed = ops.where(scores == -float("inf"), dtype_to_min(scores.dtype), scores_processed)
...
class ExponentialDecayLengthPenalty(LogitsProcessor):
    ...
    >>> set_seed(1)
    >>> outputs = model.generate(Tensor(inputs.input_ids), do_sample=True, temperature=0.9, max_length=30, pad_token_id=50256)
    >>> print(tokenizer.batch_decode(outputs)[0])
...
class LogitNormalization(LogitsProcessor):
    ...
    >>> outputs = model.generate(Tensor(inputs.input_ids), return_dict_in_generate=True, output_scores=True)
    ...
    >>> outputs = model.generate(Tensor(inputs.input_ids), renormalize_logits=True, return_dict_in_generate=True, output_scores=True)
    >>> print(np.allclose(ops.sum(ops.exp(outputs.scores[-1])).asnumpy(), np.array((1.000,)), rtol=1e-4))
    True
    ```
    """
...
class SuppressTokensAtBeginLogitsProcessor(LogitsProcessor):
    ...
    >>> outputs = model.generate(
    ...     Tensor(inputs.input_ids), return_dict_in_generate=True, output_scores=True, begin_suppress_tokens=None
    ... )
    >>> print(outputs.scores[0][0, 50256])
    tensor(11.2027)
    ```
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        vocab_tensor = ops.arange(scores.shape[-1])
        suppress_token_mask = ms.numpy.isin(vocab_tensor, self.begin_suppress_tokens)
        scores_processed = scores
...
class SuppressTokensLogitsProcessor(LogitsProcessor):
    ...
    >>> outputs = model.generate(Tensor(inputs.input_ids), return_dict_in_generate=True, output_scores=True, suppress_tokens=None)
...
class WhisperTimeStampLogitsProcessor(LogitsProcessor):
    ...
    >>> generated_ids = model.generate(inputs=Tensor(input_features), return_timestamps=True)
    ...
    >>> generated_ids = model.generate(inputs=Tensor(input_features),return_timestamps=False)
...
class UnbatchedClassifierFreeGuidanceLogitsProcessor(LogitsProcessor):
    ...
    >>> out = model.generate(Tensor(inputs["input_ids"]), guidance_scale=2, negative_prompt_ids=Tensor(neg_inputs["input_ids"]))
    >>> tokenizer.batch_decode(out, skip_special_tokens=True)[0]
    'Today, a dragon flew over Paris, France, killing at least 130 people. French media reported that'

    >>> # with a positive prompt
    >>> neg_inputs = tokenizer(["A very happy event happened,"], return_tensors="np")
    >>> out = model.generate(Tensor(inputs["input_ids"]), guidance_scale=0, negative_prompt_ids=Tensor(neg_inputs["input_ids"]))
    >>> tokenizer.batch_decode(out, skip_special_tokens=True)[0]
...
class WatermarkLogitsProcessor(LogitsProcessor):
    ...
    >>> # normal generation
    >>> out = model.generate(Tensor(inputs["input_ids"]), max_length=20, do_sample=False)
    >>> tokenizer.batch_decode(out, skip_special_tokens=True)[0]
    ...
    >>> out = model.generate(Tensor(inputs["input_ids"]), watermarking_config=watermarking_config, max_length=20, do_sample=False)
    >>> tokenizer.batch_decode(out, skip_special_tokens=True)[0]
    ...
    def __init__(
        ...
        self.fixed_table = ops.randperm(self.table_size, seed=self.rng.initial_seed()) #, generator=self.rng) # !Different from torch
        print("mindspore.ops.randperm is an experimental API that is subject to change or deletion")
    ...
    def _get_greenlist_ids(self, input_seq: ms.Tensor) -> ms.Tensor:
        ...
        vocab_permutation = ops.randperm(self.vocab_size, seed=self.rng.initial_seed()) # generator=self.rng)
    ...
    def _score_rejection_sampling(self, input_seq: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        print("Currently in mindspore ops.sort, the data types float16, (u)int8, int16, int32, int64 are well supported. \n If use float32, it may cause loss of accuracy.")
