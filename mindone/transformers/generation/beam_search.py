...
class BeamSearchScorer(BeamScorer):
    ...
    def finalize(
        ...
        # decoded: torch.Tensor = input_ids.new(batch_size * self.num_beam_hyps_to_keep, sent_max_len) # random init

        if len(best_indices) > 0 and best_indices[0] is not None:
            # indices: ms.Tensor = input_ids.new_zeros(batch_size * self.num_beam_hyps_to_keep, sent_max_len)
            indices = ops.fill(input_ids.dtype, (batch_size * self.num_beam_hyps_to_keep, sent_max_len), -1)
        else:
            indices = None
        ...
        if sent_lengths.min().item() != sent_lengths.max().item():
            ...
            decoded = ops.fill(input_ids.dtype, (batch_size * self.num_beam_hyps_to_keep, sent_max_len), pad_token_id)
        else:
...
class ConstrainedBeamSearchScorer(BeamScorer):
    ...
    def finalize(
        ...
        if sent_lengths.min().item() != sent_lengths.max().item():
            ...
            indices = ops.fill(input_ids.dtype, (batch_size * self.num_beam_hyps_to_keep, sent_max_len), pad_token_id)
        else:
            decoded: ms.Tensor = input_ids.new_zeros((batch_size * self.num_beam_hyps_to_keep, sent_max_len))
