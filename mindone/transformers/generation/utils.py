...
from ..cache_utils import ( # Convereted some in MS version
    Cache,
    DynamicCache,
    EncoderDecoderCache,
    # OffloadedCache,
...
from transformers.generation.configuration_utils import ( 
    # NEED_SETUP_CACHE_CLASSES_MAPPING, # Cache use 
    # QUANT_BACKEND_CLASSES_MAPPING, # Cache use
    GenerationConfig,
    GenerationMode,
)
...
class GenerationMixin:
    ...
    def _maybe_initialize_input_ids_for_generation(
        ...
        if "inputs_embeds" in model_kwargs:
            return ops.ones((batch_size, 0), dtype=ms.int64)

        if bos_token_id is None:
            raise ValueError("`bos_token_id` has to be defined when no `input_ids` are provided.")

        return ops.ones((batch_size, 1), dtype=ms.int64) * bos_token_id
    ...
    def _prepare_attention_mask_for_generation(
        ...
        default_attention_mask = ops.ones(inputs.shape[:2], dtype=ms.int64)
        if pad_token_id is None:
            return default_attention_mask

        is_input_ids = len(inputs.shape) == 2 and inputs.dtype in [ms.int64, ms.int64]
        if not is_input_ids:
            return default_attention_mask

        is_pad_token_in_inputs = (pad_token_id is not None) and ms.numpy.isin(inputs, pad_token_id).any()
        is_pad_token_not_equal_to_eos_token_id = (eos_token_id is None) or ~(ms.numpy.isin(eos_token_id, pad_token_id).any())
        can_infer_attention_mask = is_pad_token_in_inputs * is_pad_token_not_equal_to_eos_token_id
        attention_mask_from_padding = inputs.ne(pad_token_id).long() # difference map
    ...
    def _expand_inputs_for_generation(
        ...
        **model_kwargs,
        ...
        # Do not call mindspore.ops.repeat_interleave if expand_size is 1 because it clones
    ...
    def _update_model_kwargs_for_generation(
        ...
        if not is_encoder_decoder:
            ...
            if "attention_mask" in model_kwargs:
                ...
                ) # TODO: Bug to be resolved in MindSpore 2.3.1
        else:
            # update decoder attention mask
            if "decoder_attention_mask" in model_kwargs:
                decoder_attention_mask = model_kwargs["decoder_attention_mask"]
                model_kwargs["decoder_attention_mask"] = ops.cat(
                    [decoder_attention_mask, decoder_attention_mask.new_ones((decoder_attention_mask.shape[0], 1), dtype=decoder_attention_mask.dtype)],
                    axis=-1,
                )
    ...
    def compute_transition_scores(
        ...
        >>> outputs = model.generate(Tensor(inputs.input_ids), max_new_tokens=5, return_dict_in_generate=True, output_scores=True)
        ...
        if beam_indices is None:
            ...
            beam_indices = beam_indices.broadcast_to((-1, len(scores)))

        # 2. reshape scores as [batch_size*vocab_size, # generation steps] with # generation steps being
        # seq_len - input_length
        scores = ops.stack(scores).reshape(len(scores), -1).transpose(0, 1)
    ...
    def _validate_model_class(self):
        ...
        # `MSPreTrainedModel`. With that inheritance removed, all model classes inheriting from `GenerationMixin` can
    ...
    def _validate_model_kwargs(self, model_kwargs: Dict[str, Any]):
        ...
        # `kwargs`/`model_kwargs` is often used to handle optional construct pass inputs like `attention_mask`. If
    ...
    def _get_initial_cache_position(self, input_ids, model_kwargs):
        ...
        cache_position.to(input_ids.dtype)
    ...
    def _prepare_cache_for_generation(
        ...
        # TODO (susan): not yet support static caches
    ...
    def _supports_num_logits_to_keep(self) -> bool:
        ...
        """
        return "num_logits_to_keep" in set(inspect.signature(self.construct).parameters.keys())
    ...
    def _prepare_special_tokens(
        ...
        if (
            ...
            and ms.numpy.isin(eos_token_tensor, pad_token_tensor).any()
        ):
            if kwargs_has_attention_mask is not None and not kwargs_has_attention_mask:
                logger.warning_once(
    ...
    def generate(
        ...
        **kwargs,
        ...
        # 7. Prepare the cache.    # TODO: not support yet
        ...
        if ( #and not is_torchdynamo_compiling()
    ...
    def _dola_decoding(
        ...
        **model_kwargs,
        ...
        unfinished_sequences = ops.ones((batch_size), dtype=ms.int64)
        ...
        while self._has_unfinished_sequences(this_peer_finished):
            ...
            unfinished_sequences = unfinished_sequences & (~stopping_criteria(input_ids, scores)).int()
    ...
    def _contrastive_search(
        ...
        **model_kwargs,
        ...
        cosine_matrix_mask = ops.ones_like(input_ids, dtype=ms.int64)
        if self.config.is_encoder_decoder:
        ...
        while self._has_unfinished_sequences(this_peer_finished):
            ...
            cosine_matrix_mask = ops.cat(
                [cosine_matrix_mask, cosine_matrix_mask.new_ones((cosine_matrix_mask.shape[0], 1), dtype=cosine_matrix_mask.dtype)], axis=-1
            )
    ...
    def _beam_search(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished):
            ...
            if do_sample:
                probs = ops.softmax(next_token_scores, axis=-1)
                next_tokens = ops.multinomial(probs, num_samples=n_tokens_to_keep, replacement = False)
    ...
    def _group_beam_search(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished):
            ...
            reordering_indices = ops.zeros(batch_size * num_beams, dtype=ms.int64)
            ...
            if "cache_position" in candidate_kwargs:
                candidate_kwargs["cache_position"] = ops.cat(
                    (
                        ...
                        ops.arange(cur_len, cur_len + candidate_length, dtype=ms.int64),
                    ),
                    axis=0,
                )
...
def _speculative_sampling(
    ...
    if is_done_candidate and n_matches == candidate_length:
        ...
        if n_matches < gamma:
            ...
            p_prime = p_n_plus_1
        t = ops.multinomial(p_prime, num_samples=1, replacement = False).squeeze(1)[None, :]
