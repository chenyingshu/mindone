...
class StopStringCriteria(StoppingCriteria):
    ...
    >>> gen_out = model.generate(Tensor(inputs.input_ids), stop_strings=["Texas"], tokenizer=tokenizer)
    >>> print(tokenizer.batch_decode(gen_out, skip_special_tokens=True)[0])
    The biggest states in the USA by land area:
    - Alaska
    - Texas
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor, **kwargs) -> ms.Tensor:
        ...
        unflatten_op = nn.Unflatten(-1, (self.num_stop_strings, -1))
        ...
        valid_positions = unflatten_op(valid_positions)
        ...
        end_lengths = unflatten_op(end_lengths)
