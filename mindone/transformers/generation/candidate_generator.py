...
class CandidateGenerator:
    ...
    def get_candidates(self, input_ids: ms.Tensor) -> Tuple[ms.Tensor, Optional[ms.Tensor]]:
        ...
        Return:
            `ms.Tensor` of shape `(batch_size, candidate_length)` containing the candidate sequences to be
            assessed by the model and, optionally, a `ms.Tensor` of shape `(batch_size, candidate_length,
    ...
    def update_candidate_strategy(self, input_ids: ms.Tensor, scores: ms.Tensor, num_matches: int):
        """
        ...
        Args:
            input_ids (`ms.Tensor` of shape `(batch_size, sequence_length)`):
                Indices of input sequence tokens in the vocabulary. [What are input IDs?](../glossary#input-ids)
            scores (`ms.Tensor` of shape `(batch_size, candidate_length, config.vocab_size)`):
...
class AssistedCandidateGenerator(CandidateGenerator):
    ...
    Args:
        ...
        model_kwargs (`Dict`):
            ...
            model as well.
        inputs_tensor (`ms.Tensor`, *optional*):
    ...
    def __init__(
        ...
        for key, value in model_kwargs.items():  # deepcopy crashes if we attempt to copy encoder outputs with grads
            if key not in ("encoder_outputs", "assistant_encoder_outputs", "past_key_values"):
                assistant_kwargs[key] = (
                    value if isinstance(value, ms.Tensor) else copy.deepcopy(value)
                )
        ...
        if assistant_model.config.is_encoder_decoder:
            ...
            self.assistant_kwargs["attention_mask"] = self.assistant_kwargs.get(
                ...
                ops.ones((input_ids.shape[0], 1), dtype=ms.int32),
            )
        else:
    ...
    def get_candidates(self, input_ids: ms.Tensor) -> Tuple[ms.Tensor, Optional[ms.Tensor]]:
        ...
        Return:
            `ms.Tensor` of shape `(batch_size, candidate_length)` containing the candidate sequences to be
            assessed by the model and a `ms.Tensor` of shape `(batch_size, candidate_length,
            vocabulary_size)` containing the logits associated to each candidate.
        """
        ...
        candidate_logits = ops.stack(assistant_output.scores, axis=1)
...
class PromptLookupCandidateGenerator(CandidateGenerator):
    ...
    def get_candidates(self, input_ids: ms.Tensor) -> Tuple[ms.Tensor, Optional[ms.Tensor]]:
        ...
        for ngram_size in range(min(self.max_matching_ngram_size, input_length - 1), 0, -1):
            ...
            matches = (windows == ngram_tensor).all(axis=2)
            ...
            for idx in match_indices:
                ...
                if start_idx < end_idx:
                    ...
                    mask = ms.numpy.isin(chosen_ids, self.eos_token_id)
        ...
        candidate_input_ids = ops.cat((input_ids, chosen_ids), axis=1)
...
def _prepare_token_type_ids(model_kwargs: Dict[str, Any], new_length: int) -> Dict[str, Any]:
    ...
    if type_length_diff < 0:
        ...
        model_kwargs["token_type_ids"] = ops.cat([model_kwargs["token_type_ids"], token_type_copies], axis=-1)
    return model_kwargs